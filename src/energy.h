...
//#pragma omp parallel for reduction(+ : u) if (omp_enable and omp_i2all)
    ...
    virtual double g2g(const Tgroup &g1, const Tgroup &g2) {
        ...
        if (!cut(g1, g2)) {
            ...
            // #pragma omp parallel for collapse(2) default(none) shared(G1, G2, g1, g2) reduction(+: u) schedule(dynamic, 100) if (G1*G2 >= 200)
    ...
    virtual double g2g(const Tgroup &g1, const Tgroup &g2, const std::vector<int> &index1) {
        ...
        if (!cut(g1, g2)) {
            ...
            const size_t G2 = g2.size();
            // https://gcc.gnu.org/gcc-9/porting_to.html#ompdatasharing
            // #pragma omp parallel for collapse(2) default(none) shared(I1, G2, index1, g1, g2) reduction(+: u) schedule(dynamic, 100) if (G1*G2 >= 200)
